...
class DfjsptMaEnv(MultiAgentEnv):
    """
    A Hierarchical Multi-agent Environment for Dynamic Flexible Job-shop Scheduling Problem with Transportation
    """
    ...
    def __init__(self, env_config):
        ...
        makespan_upper_bound = 15 * MAX_JOBS * MAX_MACHINES * dfjspt_params.max_prcs_time
        ...
        self.agents = self.possible_agents = ["agent0", "agent1", "agent2"]
    ...
    def reset(self, seed=None, options=None):
        ...
        func("before_instance")

        self.instance = SchedulingInstance(
            seed=52 + self.current_instance_id,
            n_jobs=generate_n_jobs,
            n_machines=dfjspt_params.n_machines,
        )
        factory_graph = FactoryGraph(dfjspt_params.n_machines)
        self.n_operations_for_jobs = self.instance.n_operations_for_jobs
        self.job_arrival_time = self.instance.job_arrival_time
        self.job_due_date = self.instance.job_due_date
        # self.processing_time_matrix = instance.processing_time_matrix
        self.empty_moving_time_matrix = factory_graph.unload_transport_time_matrix * dfjspt_params.tspt_time_factor
        self.transport_time_matrix = factory_graph.loaded_transport_time_matrix * dfjspt_params.tspt_time_factor
        self.machine_quality = np.ones((1, dfjspt_params.n_machines), dtype=float)
        self.transbot_quality = np.ones((1, dfjspt_params.n_transbots), dtype=float)
        ...
        for job_id in range(self.n_jobs):
            for operation_id in range(self.n_operations_for_jobs[job_id]):
                processing_time_of_operations = self.instance.jobs[job_id].operations_matrix[operation_id][
                    np.where(self.instance.jobs[job_id].operations_matrix[operation_id] > 0)]
        ...
        self.rule_makespan_for_current_instance = 1.5 * MAX_JOBS * MAX_MACHINES * dfjspt_params.max_prcs_time
        # del instance
        ...
        self.transbot_routes = {t_id: np.empty((0, 5), dtype=int) for t_id in range(self.n_transbots)}
    ...
    def step(self, action):
        ...
        if self.stage == 0:
            ...
            for machine_id in range(self.n_machines):
                ...
                self.machine_features[machine_id, 5] = max(-1.0, self.instance.jobs[self.chosen_job].operations_matrix[
                    self.operation_id, machine_id] / self.machine_features[machine_id, 1])
            ...
            if self.schedule_done:
                ...
                func("before_global_schedule")
                global_schedule_file = os.path.dirname(os.path.abspath(__file__)) \
                    + f"/InterfaceWithLocal/global_schedules/M{dfjspt_params.n_machines}T{dfjspt_params.n_transbots}" \
                    + f"/global_schedule_J{dfjspt_params.n_jobs}" \
                    + f"I{self.current_instance_id}.pkl"
                self.global_schedule = convert_schedule_to_class(
                        ...
                        transbot_routes=self.transbot_routes
                )
                func("after_global_schedule")
    ...
    # *********************************
    def _schedule_tspt_task(self, job_id: int, task_id: int, operation_id: int, transbot_id: int, machine_id: int) -> dict:
        """
        ...
        new_task = np.array([[job_id, operation_id, transbot_location, job_location, machine_location]], dtype=int)
        previous_operation_finish_time = float(self.job_features[job_id, 2])

        # todo: what if `job_location == machine_location`, which means this job doesn't need transportation?
        if job_location == machine_location:
            actual_duration = 0
        ...
        if len_transbot_routes:
            if self.perform_left_shift_if_possible:
                ...
                transbot_first_task_start_time = self.result_start_time_for_jobs[
                    self.transbot_routes[transbot_id][0][0],
                    self.transbot_routes[transbot_id][0][1], 0]

                if j_lower_bound_ft <= transbot_first_task_start_time:
                    self.transbot_routes[transbot_id] = np.insert(self.transbot_routes[transbot_id], 0, new_task, axis=0)
                    start_time = previous_operation_finish_time
                    finish_time = start_time + actual_duration
                    ...
                    self.transbot_routes[transbot_id] = np.append(self.transbot_routes[transbot_id], new_task, axis=0)
                ...
                for i, (m_prev, m_next) in enumerate(
                    ...
                    m_temp_prev_ft = self.result_finish_time_for_jobs[m_prev[0], m_prev[1], 0]
                    m_temp_next_st = self.result_start_time_for_jobs[m_next[0], m_next[1], 0]
                    ...
                    self.transbot_routes[transbot_id] = np.insert(self.transbot_routes[transbot_id], i + 1, new_task, axis=0)
            ...
            # self.transbot_routes[transbot_id] = np.insert(self.transbot_routes[transbot_id], 0, new_task)
            self.transbot_routes[transbot_id] = np.append(self.transbot_routes[transbot_id], new_task, axis=0)
            start_time = previous_operation_finish_time
            finish_time = start_time + actual_duration
    ...
    def _schedule_prcs_task(self, job_id: int, task_id: int, operation_id: int, machine_id: int) -> dict:
        ...
        :param operation_id:    the operation of the chosen job that shall be scheduled.
        ...
        expected_duration = self.instance.jobs[job_id].operations_matrix[self.Graph.nodes[task_id]["operation_id"], machine_id]
        ...
        if len_machine_routes:
            if self.perform_left_shift_if_possible:
                ...
                if j_lower_bound_ft <= machine_first_task_start_time:
                    ...
                    # Fix: machine_features[3] should store the finish time of the LAST task in the route
                    # When inserting in the middle, we should find the actual last task's finish time
                    last_task_in_route = self.machine_routes[machine_id][-1]  # Get the last task after insertion
