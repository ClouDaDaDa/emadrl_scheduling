"""
Training script for Non-Embodied Multi-Agent Scheduling

This script trains the non-embodied scheduling environment using PPO.
It follows the same structure as the embodied training script but with
simplified configurations suitable for the event-driven, non-physical environment.
"""
...
from local_realtime_scheduling.BaselineMethods.NonEmbodiedScheduling.NonEmbodiedSchedulingMultiAgentEnv import NonEmbodiedSchedulingMultiAgentEnv
...
# Add curriculum learning support (optional for non-embodied)
@ray.remote
class NonEmbodiedCurriculumTaskManager:
    def __init__(self,
                 task_id_min,
                 task_id_max,
                 task_range,
...
def train_non_embodied_pipeline(
    ...
    *,
    ...
    """Run the non-embodied training pipeline with the given configuration."""
    if args is None:
    ...
    log_dir = (os.path.dirname(os.path.abspath(__file__))
        + f'/ray_results_non_embodied/M{dfjspt_params.n_machines}T{dfjspt_params.n_transbots}W{dfjspt_params.time_window_size}')
...
if __name__ == "__main__":
    ...
    # Create argument parser with non-embodied specific defaults
    parser = add_default_args(
        default_iters=2000,
        default_reward=1000,
    )
    ...
    # Environment configuration (no physical constraints)
    env_config = {
        "n_machines": dfjspt_params.n_machines,
        "n_transbots": dfjspt_params.n_transbots,  # Simplified transbots (no charging, pathfinding)
        "factory_instance_seed": dfjspt_params.factory_instance_seed,
    }

    # Create example environment for space definitions
    example_env = NonEmbodiedSchedulingMultiAgentEnv(env_config)
    ...
    if dfjspt_params.enable_curriculum:
        ...
        if len(local_schedule_files) > 0:
            ...
            curriculum_task_manager = NonEmbodiedCurriculumTaskManager.options(name="non_embodied_curriculum_task_manager").remote(
                task_id_min=0,
    ...
    # Calculate batch sizes (event-driven episodes are typically shorter)
    # Based on validation results: ~50 event steps for 62 operations
    estimated_episode_length = 500  # Conservative estimate for event steps
    train_batch_size = max(1, dfjspt_params.num_env_runners) * estimated_episode_length

    # Model configuration for non-embodied scheduling
    if dfjspt_params.use_lstm:
        model_config = {
            "lstm_cell_size": 64,
            "use_lstm": True,
            "max_seq_len": (example_env.num_machines + example_env.num_transbots) * 3,  # Shorter for event-driven
        }
    else:
        model_config = {
            "fcnet_hiddens": [256, 256],  # Fully connected layers for global observations
            "fcnet_activation": "relu",
        }

    # PPO configuration for non-embodied scheduling
    base_config = (
        PPOConfig()
        .environment(
            env=NonEmbodiedSchedulingMultiAgentEnv,
        ...
        .env_runners(
            ...
            # No observation filter needed for global features
        )
        .training(
            train_batch_size_per_learner=train_batch_size,
            minibatch_size=estimated_episode_length,
            entropy_coeff=0.001,
            num_epochs=5,
            # Learning rate schedule (adjusted for event-driven episodes)
            lr=[
                [0, 5e-5],
        ...
        .rl_module(
            rl_module_spec=MultiRLModuleSpec(
                rl_module_specs={
                    # Machine policy for non-embodied scheduling
                    ...
                    # Transbot policy for non-embodied scheduling (simplified)
        ...
        .multi_agent(
            # Both machine and transbot agents in non-embodied version
    ...
    print("=== Non-Embodied Scheduling Training Configuration ===")
    ...
    print(f"Environment: Event-driven, no physical constraints")
    print(f"Agents: {len(example_env.machine_agents)} machines + {len(example_env.transbot_agents)} transbots (simplified)")
    print(f"Policies: p_machine, p_transbot")
    print("=" * 50)

    # Run the training
    train_non_embodied_pipeline(base_config, args)
