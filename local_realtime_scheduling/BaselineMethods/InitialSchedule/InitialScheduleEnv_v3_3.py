...
EPISODE_TIME_UPPER_BOUND = dfjspt_params.episode_time_upper_bound
...
class InitialScheduleEnv(MultiAgentEnv):
    ...
    Key Differences from LocalSchedulingMultiAgentEnv:
    - Agents are constrained to follow predefined task queues from the initial local schedule
    - Machine agents can only process jobs in their processing_tasks_queue_for_current_time_window
    - Transbot agents can only transport jobs in their transport_tasks_queue_for_current_time_window
    - Task queues are populated during reset() based on the initial schedule
    - Action masking ensures agents only act on scheduled tasks at the correct time
    - Task queue management: tasks are removed when started (not when finished)
    - More restrictive action validation with penalties for schedule violations

    Fixes Applied:
    1. Machine observation action masking now properly checks scheduled start times
    2. Transbot observation action masking now properly validates timing and job readiness
    3. Task queue removal timing corrected (remove when task starts, not when finished)
    4. Penalty logic made consistent with original environment (using total_n_ops_for_curr_tw)
    5. Action handling now validates against scheduled task queues with penalties for violations
    6. Proper timing validation for both machines and transbots
    """
    ...
    def __init__(self,
        ...
        self.num_machine_actions = self.n_jobs_handled_by_machine + 4 + 1
    ...
    def _initialize_state(self):
        ...
        return {machine_id: self._get_machine_obs(machine_id) for machine_id in self.machine_agents}
    ...
    #
    #     # # Generate a random score for each job
    #     # job_scores = np.random.random(self.num_jobs)  # Example: random scores between 0 and 1
    #
    ...
    #         else:
    #             job_scores[job_id] = (
    #                     - this_job.operations_matrix[current_op, machine_index] / MAX_PRCS_TIME
    #                     - this_job.job_progress_for_current_time_window
    #                     - unload_time_matrix[location_map[this_job.current_location], machine_index] / MAX_PRCS_TIME
    #             )
    #
    ...
    def _get_machine_obs(self, machine_agent_id):
        ...
        if machine.machine_status == 3:
            ...
            if machine.current_processing_task is None:
                if len(machine.processing_tasks_queue_for_current_time_window) > 0:
                    # Check if it's time to start the next scheduled task
                    next_task = machine.processing_tasks_queue_for_current_time_window[0]
                    job_id, operation_id, scheduled_start_time = next_task

                    # Check if this is the correct operation to process next
                    if operation_id == self.scheduling_instance.jobs[job_id].current_processing_operation:
                        self.jobs_handled_by_machines[machine_index][0] = job_id
                        job_action_index = 0
                        machine_action_mask[job_action_index] = 1
                    else:
                        # Wrong operation timing, do nothing
                        machine_action_mask[self.n_jobs_handled_by_machine + 4] = 1

                else:
                    # No scheduled tasks, do nothing
                    machine_action_mask[self.n_jobs_handled_by_machine + 4] = 1
            else:
                # Already has a task assigned, do nothing
                machine_action_mask[self.n_jobs_handled_by_machine + 4] = 1
        ...
        machine_features = np.array([
            ...
            machine.estimated_remaining_time_to_finish,  # [2] time to finish the current task
            machine.cumulative_tasks,  # [3] cumulative number of tasks
            self.initial_estimated_makespan - self.current_time_after_step,  # [4] The overall time progress
            self.remaining_operations,  # [5] Global feature of the number of remaining operations
        ], dtype=np.float32)
    ...
    #         # job_scores[job_id] = (-1.0 * (transbot_to_job + job_to_machine) / MAX_PRCS_TIME
    #         #                       - 1.0 * this_job.job_progress_for_current_time_window)
    #
    #     # Rank jobs based on their score, jobs with higher score are more prioritized
    #     job_index_by_score = sorted(job_scores.keys(), key=lambda j: job_scores[j], reverse=True)
    ...
    def _get_transbot_obs(self, transbot_agent_id):
        ...
        if transbot.agv_status == 0:
            if transbot.is_for_charging:
                # If marked for charging, only do nothing
                transbot_action_mask[self.n_jobs_handled_by_transbot + 1] = 1
            else:
                if len(transbot.transport_tasks_queue_for_current_time_window) > 0:
                    # Check if it's time to start the next scheduled transport task
                    next_task = transbot.transport_tasks_queue_for_current_time_window[0]
                    job_id, operation_id, scheduled_start_time = next_task

                    # # Check if the scheduled start time has arrived or passed
                    # if self.current_time_after_step >= scheduled_start_time:
                    # Check if this is the correct operation and the job needs transportation
                    current_job = self.scheduling_instance.jobs[job_id]
                    if (operation_id == current_job.current_processing_operation and
                            # current_job.assigned_machine is not None and
                            current_job.assigned_transbot is None):
                        self.jobs_handled_by_transbots[transbot_index][0] = job_id
                        job_action_index = 0
                        transbot_action_mask[job_action_index] = 1
                    else:
                        # Job not ready for transport or timing issue, do nothing
                        transbot_action_mask[self.n_jobs_handled_by_transbot + 1] = 1
                    # else:
        ...
        #     ]
        #
        #     if transbot.agv_status in (0, 1) and not transbot.is_for_charging:
        ...
        transbot_features = np.array([
            ...
            # [3] time since previous get a positive reward
            transbot.t_since_prev_r,
            # [4] Global feature of the overall time progress
            self.initial_estimated_makespan - self.current_time_after_step,
            # [5] Global feature of the number of remaining operations
    ...
    def reset(self, seed=None, options=None):
        ...
        # if options is None:
        #     raise ValueError(f"Local schedule, factory instance and scheduling instance not found in options!")
        ...
        self.time_upper_bound = EPISODE_TIME_UPPER_BOUND
        self.reward_this_step = 0.0
        self.total_cost = 0.0
        ...
        for job_id, local_schedule_job in self.local_schedule.jobs.items():
            ...
            for operation_id in local_schedule_job.operations:
                if local_schedule_job.operations[operation_id].is_current_window:
                    ...
                    self._assign_initial_schedule_for_agents(local_schedule_job, operation_id)
    ...
    def _insert_machine_tasks_queue(self, local_schedule_job, operation_id, local_schedule_job_operation):
        # (job, operation, start_time)
        scheduled_processing_op = (local_schedule_job.job_id, operation_id,
                                   local_schedule_job_operation.scheduled_start_processing_time)
    ...
    def _insert_transbot_tasks_queue(self, local_schedule_job, operation_id, local_schedule_job_operation):
        # (job, operation, start_time)
        scheduled_transporting_op = (local_schedule_job.job_id, operation_id,
                                     local_schedule_job_operation.scheduled_start_transporting_time)

        if local_schedule_job_operation.scheduled_start_transporting_time != local_schedule_job_operation.scheduled_finish_transporting_time:
    ...
    def _assign_initial_schedule_for_agents(self, local_schedule_job, operation_id):
        local_schedule_job_operation = local_schedule_job.operations[operation_id]
        self._insert_machine_tasks_queue(local_schedule_job, operation_id, local_schedule_job_operation)

        if local_schedule_job_operation.assigned_transbot is not None:
            self._insert_transbot_tasks_queue(local_schedule_job, operation_id, local_schedule_job_operation)
    ...
    def step(self, action_dict):
        ...
        # Decision Stage 1: Transbots should make decisions
            ...
            # if self.current_time_after_step - self.local_result.time_window_start > 1000:
            #     print("too many steps!")

            # self.reward_this_step -= self.current_time_after_step / self.initial_estimated_makespan
            self.reward_this_step -= 1.0 / (self.initial_estimated_makespan - self.local_result.time_window_start)
            ...
            # if self.remaining_operations == 0 and self.has_saved_instance_snapshot is False:
            #     # self.remaining_operations = None
            #     for job_id in self.local_schedule.jobs:
            #         this_job = self.scheduling_instance.jobs[job_id]
            #         this_job.p_ops_for_cur_tw = []
            #         # this_job.n_p_ops_for_curr_tw = 0
            #     self._save_instance_snapshot()
            #     self.has_saved_instance_snapshot = True
        ...
        if current_machine.machine_status == 0:
            ...
            if current_machine.current_maintenance_method is not None:
                ...
                if current_job.job_status == 0 and current_machine.machine_status == 0 and current_job.current_location == current_machine.location:
                    ...
                    # Remove task from queue when processing starts (not when it finishes)
                    # This is the correct timing for the InitialScheduleEnv
                    if len(current_machine.processing_tasks_queue_for_current_time_window) > 0:
                    ...
                    if self._check_machine_finish_task(machine_id=machine_index):
                        ...
                        self.rewards[f'machine{current_machine.machine_id}'] += 1.0 * (
                                MAX_PRCS_TIME - actual_processing_duration) / MAX_PRCS_TIME
            ...
            if current_transbot.current_task is not None:
                ...
                # for transporting
                    ...
                    if current_job.assigned_machine is None:
                        assigned_machine_id = self.local_schedule.jobs[current_job.job_id].operations[
                            current_job.current_processing_operation].assigned_machine
                    else:
                        assigned_machine_id = current_job.assigned_machine
                    machine_location = self.factory_instance.factory_graph.pickup_dropoff_points[
                        self.factory_instance.machines[assigned_machine_id].location]
                    ...
                    if transbot_to_job == 0:  # if the transbot is at the same location with the job:
                        if current_job.job_status == 0 and current_job.assigned_machine is not None:
                            if len(current_job.scheduled_results) > 0 and self.current_time_before_step < current_job.scheduled_results[-1][3]:
                                ...
                                # # Remove task from queue when loaded transport starts
                                # # This is the correct timing for the InitialScheduleEnv
                                # if len(current_transbot.transport_tasks_queue_for_current_time_window) > 0:
                                #     current_transbot.transport_tasks_queue_for_current_time_window.pop(0)
    ...
    def _check_machine_processing_action(self, machine_index, processing_action):
        ...
        if current_job.assigned_machine is not None:
            self.rewards[f'machine{machine_index}'] -= 0.2
    ...
    def _handle_machine_action(self, machine_index, current_machine, action):
        # In InitialScheduleEnv, machines should follow their predefined schedule
        ...
        if 0 <= action < self.n_jobs_handled_by_machine:
            ...
            # Additional check: ensure this job is the next scheduled task
            if (len(current_machine.processing_tasks_queue_for_current_time_window) > 0 and
                    current_machine.processing_tasks_queue_for_current_time_window[0][0] == job_id):
                ...
                # Trying to process a job that's not next in schedule - penalty
                # self.rewards[f'machine{machine_index}'] -= 0.1
                raise Exception(f"Machine {machine_index} is trying to process a job that's not next in schedule!")
    ...
    def _check_transbot_transporting_action(self, transbot_index, transporting_action):
        ...
        if current_job.assigned_machine is None:
            assigned_machine_id = self.local_schedule.jobs[current_job.job_id].operations[current_job.current_processing_operation].assigned_machine
        ...
        job_to_machine = self.factory_instance.factory_graph.unload_transport_time_matrix[
            job_location_index,
            # current_job.assigned_machine  # machine_location_index == machine_index
            assigned_machine_id
        ]
        ...
        if current_job.assigned_transbot is not None:
            ...
            # Apply penalty for trying to take an already assigned job - consistent with original environment
    ...
    def _handle_transbot_action(self, transbot_index, current_transbot, action):
        # In InitialScheduleEnv, transbots should follow their predefined schedule
        ...
        if 0 <= action < self.n_jobs_handled_by_transbot:
            ...
            # Additional check: ensure this job is the next scheduled transport task
            if (len(current_transbot.transport_tasks_queue_for_current_time_window) > 0 and
                    current_transbot.transport_tasks_queue_for_current_time_window[0][0] == job_id):
                ...
                # Trying to transport a job that's not next in schedule - penalty
                # self.rewards[f'transbot{transbot_index}'] -= 0.1
                raise Exception(f"Transbot {transbot_index} is trying to transport a job that's not next in schedule!")
    ...
    def _log_episode_termination(self):
        ...
        # for machine in self.factory_instance.machines:
        #     if machine.start_time_of_the_task is not None and machine.actual_processing_duration is not None:
        #         if makespan < machine.start_time_of_the_task + machine.actual_processing_duration:
        #             makespan = machine.start_time_of_the_task + machine.actual_processing_duration
    ...
    def _save_instance_snapshot(self, final=False, policy_name="INITIAL", episode_id=None, instance_snapshot_dir=None):
    ...
    def _initialize_rendering(self):
        ...
        self.machine_color_map = {m_id: colormap(i / max(1, self.num_machines - 1)) for i, m_id in enumerate(machine_ids)}
        ...
        self.transbot_color_map = {t_id: colormap(i / max(1, self.num_transbots)) for i, t_id in enumerate(transbot_ids)}
    ...
    def plot_transbot_trajectory(self, transbot_id, start_time, end_time):
        # Filter the scheduled results for the specified time range
        filtered_results = {t: loc for t, loc in self.factory_instance.agv[transbot_id].trajectory_hist.items() if
                            start_time <= t <= end_time}

        if not filtered_results:
            print(f"No data available for the time range {start_time} to {end_time}.")
            return

        # Extract the time steps and corresponding locations from the filtered results
        time_steps = list(filtered_results.keys())
        locations = list(filtered_results.values())

        # Split the locations into x and y coordinates
        x_coords = [loc[0] for loc in locations]
        y_coords = [loc[1] for loc in locations]
        z_coords = time_steps  # The time steps will be mapped to the z-axis

        # Create a figure with 2 subplots: one for 2D and one for 3D
        fig = plt.figure(figsize=(14, 8))

        # 2D view plot
        ax1 = fig.add_subplot(121)  # 1 row, 2 columns, first subplot
        ax1.set_aspect('equal', adjustable='box')  # Equal aspect ratio for X and Y axes

        # Plot obstacles
        for x, y in self.factory_instance.factory_graph.obstacles:
            ax1.add_patch(pch.Rectangle((x - 0.5, y - 0.5), 1, 1, color='dimgray'))

        # Plot the trajectory with color gradient
        colormap_type = ["Blues", "Oranges", "Greens", "Purples", "PuRd"]
        norm = mcolors.Normalize(vmin=min(time_steps), vmax=max(time_steps))
        cmap = plt.cm.get_cmap(colormap_type[transbot_id % 5])

        for i in range(1, len(time_steps)):
            ax1.plot(x_coords[i - 1:i + 1], y_coords[i - 1:i + 1], color=cmap(norm(time_steps[i - 1])), lw=2)
            ax1.scatter(x_coords[i], y_coords[i], color=cmap(norm(time_steps[i])), s=50, zorder=5)

        # Mark the starting point
        ax1.scatter(x_coords[0], y_coords[0], color='g', label='Start Point', zorder=10)

        # Mark the ending point
        ax1.scatter(x_coords[-1], y_coords[-1], color='r', label='End Point', zorder=10)

        # Set title and labels
        ax1.set_title(f"Transbot 2D Trajectory from Time {start_time} to {end_time}")
        ...
        # ax2.set_aspect('equal', adjustable='box')  # Equal aspect ratio for X and Y axes

        # Plot the trajectory with color gradient
        for i in range(1, len(time_steps)):
            ax2.plot(x_coords[i - 1:i + 1], y_coords[i - 1:i + 1], z_coords[i - 1:i + 1],
                     color=cmap(norm(time_steps[i - 1])), lw=2)
            ax2.scatter(x_coords[i], y_coords[i], z_coords[i], color=cmap(norm(time_steps[i])), s=50, zorder=5)

        # Mark the starting point
        ax2.scatter(x_coords[0], y_coords[0], z_coords[0], color='g', label='Start Point', zorder=10)

        # Mark the ending point
        ax2.scatter(x_coords[-1], y_coords[-1], z_coords[-1], color='r', label='End Point', zorder=10)

        # Set title and labels
        ax2.set_title(f"Transbot 3D Trajectory from Time {start_time} to {end_time}")
    ...
    def plot_multiple_transbot_trajectories(self, transbot_id_list, start_time, end_time):
        """
        Plot the trajectories of multiple transbots in both 2D and 3D views, each with a different gradient.
        :param transbot_id_list: A list including transbot IDs.
        :param start_time: The start time for the trajectory to be plotted.
        :param end_time: The end time for the trajectory to be plotted.
        """

        max_plot_transbots = 5
        if len(transbot_id_list) > max_plot_transbots:
            raise ValueError(
                f"Please don't plot more than {max_plot_transbots} transbots' trajectories at the same time!")
        ...
        # Iterate through each transbot's results and plot its trajectory
        for transbot_id in transbot_id_list:
            # Filter the scheduled results for the specified time range
            filtered_results = {t: loc for t, loc in self.factory_instance.agv[transbot_id].trajectory_hist.items() if
                                start_time <= t <= end_time}

            if not filtered_results:
                print(f"No data available for transbot {transbot_id} in the time range {start_time} to {end_time}.")
                continue
            ...
            # Create a color gradient for each transbot
            norm = mcolors.Normalize(vmin=min(time_steps), vmax=max(time_steps))
            cmap = plt.cm.get_cmap(colormap_type[transbot_id])
            ...
            # Mark the starting point in 2D
            ax1.scatter(x_coords[0], y_coords[0], color='g', label=f'{transbot_id} Start Point', zorder=10)
            # Mark the ending point in 2D
            ax1.scatter(x_coords[-1], y_coords[-1], color='r', label=f'{transbot_id} End Point', zorder=10)
            ...
            # Mark the starting point in 3D
            ax2.scatter(x_coords[0], y_coords[0], z_coords[0], color='g', label=f'{transbot_id} Start Point', zorder=10)
            # Mark the ending point in 3D
            ax2.scatter(x_coords[-1], y_coords[-1], z_coords[-1], color='r', label=f'{transbot_id} End Point',
                        zorder=10)

        # Set title and labels for 2D plot
        ax1.set_title(f"Multiple Transbot 2D Trajectories from Time {start_time} to {end_time}")
        ax1.set_xlabel("X Coordinate")
        ax1.set_ylabel("Y Coordinate")
        x_ticks = range(-1, self.factory_instance.factory_graph.width + 1, 1)
        y_ticks = range(-1, self.factory_instance.factory_graph.height + 1, 1)
        ...
        # Set title and labels for 3D plot
        ax2.set_title(f"Multiple Transbot 3D Trajectories from Time {start_time} to {end_time}")
        ax2.set_xlabel("X Coordinate")
        ax2.set_ylabel("Y Coordinate")
        ax2.set_zlabel("Time Step")
        ax2.legend()
            ...
            for ops_id in range(0, len(transbot.scheduled_results), 2):
                ...
                if op_type == "Unload Transporting":
                    if job_id == -1:
                        ...
                        rect_height = 0.28  # Reduced to compensate for linewidth=1
                    ...
                    # Use slightly smaller height to compensate for border visual effect
                    rect_height = 0.38  # Reduced to compensate for linewidth=1
