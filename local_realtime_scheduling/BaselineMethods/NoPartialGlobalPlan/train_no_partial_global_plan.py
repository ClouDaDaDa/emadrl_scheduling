...
from local_realtime_scheduling.BaselineMethods.NoPartialGlobalPlan.NoPartialGlobalPlanCallbacks import MyCallbacks
...
@ray.remote
class NoPartialGlobalPlanCurriculumTaskManager:
    def __init__(self,
                 task_id_min,
                 task_id_max,
                 task_range,
...
def train_with_tune_pipeline(
    ...
    *,
    ...
    log_dir = (os.path.dirname(os.path.abspath(__file__))
        + f'/ray_results/M{dfjspt_params.n_machines}T{dfjspt_params.n_transbots}W{dfjspt_params.time_window_size}_NoPartial')
...
if __name__ == "__main__":
    ...
    example_env = NoPartialGlobalPlanMultiAgentEnv(env_config)

    if dfjspt_params.enable_curriculum:
        # For NoPartialGlobalPlan, we use global instances instead of local schedules
        global_instance_dir = os.path.dirname(current_dir) + \
             "/InterfaceWithGlobal/global_schedules" + \
             f"/M{dfjspt_params.n_machines}T{dfjspt_params.n_transbots}"
        
        if os.path.exists(global_instance_dir):
            # Get all .pkl files in the directory
            global_instance_files = [file_name for file_name in os.listdir(global_instance_dir) if file_name.endswith(".pkl")]

            if len(global_instance_files) > 0:
                # Sort files by the 'ops' number (from smallest to largest)
                sorted_files = sorted(global_instance_files, key=lambda f: int(re.search(r'ops(\d+)\.pkl', f).group(1)))
                curriculum_task_manager = NoPartialGlobalPlanCurriculumTaskManager.options(name="curriculum_task_manager").remote(
                    task_id_min=0,
                ...
                raise ValueError(f"Cannot find any global instance file in {global_instance_dir}!")
        else:
            raise ValueError(f"Global instance directory {global_instance_dir} does not exist!")
    ...
    base_config = (
        PPOConfig()
        .environment(
            env=NoPartialGlobalPlanMultiAgentEnv,
