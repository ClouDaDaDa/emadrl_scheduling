...
class NoPartialGlobalPlanMultiAgentEnv(MultiAgentEnv):
    """
    A Multi-agent Environment for Integrated Production, Transportation and Maintenance Real-time Scheduling
    WITHOUT Partial Global Plans guidance. Uses rough partitioning method instead.
    """
    ...
    def reset(self, seed=None, options=None):
        """
        Reset the environment for No Partial Global Plan method.
        ...
        :param options: A dict including:
            ...
            - max_window: int
            - current_window: int
            - start_t_for_curr_window: float
        :return: observations, infos
        """

        func("Env reset for No Partial Global Plan.")
        ...
        start_time = options["start_t_for_curr_window"]
        ...
        self.time_deviation = 0.0  # No global schedule reference
        self.num_jobs = options.get("instance_n_jobs", None)
        self.current_instance_id = options.get("current_instance_id", None)
        self.has_saved_instance_snapshot = False
        self.max_window = options.get("max_window", None)
        self.current_window = options.get("current_window", None)
        
        # Create rough local schedule based on segment
        self.local_schedule = self._create_rough_local_schedule()
        self.snapshot = None
        
        # Estimate makespan based on rough partitioning
    ...
    def _create_rough_local_schedule(self):
        """
        Create a rough local schedule based on current segment using rough partitioning.
        Each segment contains ALL jobs but each job only contains approximately 1/3 of its operations.
        """
        jobs = self.scheduling_instance.jobs
        if isinstance(jobs, list):
            # Convert list to dict if needed
            jobs_dict = {i: job for i, job in enumerate(jobs)}
            jobs = jobs_dict
        
        # Create rough local schedule
        local_schedule = LocalSchedule()
        local_schedule.time_window_start = self.current_time_before_step
        local_schedule.local_makespan = self._estimate_rough_makespan()
        local_schedule.n_ops_in_tw = 0
        
        # Add ALL jobs, but each job only contains operations for current segment
        for job_id, job in jobs.items():
            job_ops_count = job.operations_matrix.shape[0]
            if job_ops_count == 0:
                continue
            
            # Calculate operation range for current segment for this job
            # Windows are 0-based indexed in this env: [0, max_window-1]
            operations_per_segment = job_ops_count // self.max_window
            if self.current_window == self.max_window - 1:  # Last segment gets remaining operations
                start_op = (self.max_window - 1) * operations_per_segment
                end_op = job_ops_count
            else:
                start_op = self.current_window * operations_per_segment
                end_op = (self.current_window + 1) * operations_per_segment
            
            # Only add this job if it has operations in current segment
            if start_op < job_ops_count:
                local_job_schedule = Local_Job_schedule(job_id=job_id)
                local_job_schedule.available_time = self.current_time_before_step
                local_job_schedule.estimated_finish_time = self.current_time_before_step + dfjspt_params.time_window_size  # Rough estimate
                
                # Add only operations for current segment
                for op_idx in range(start_op, end_op):
                    operation = type('Operation', (), {
                        'operation_id': op_idx,
                        'is_current_window': True,
                        'is_addition_ops': False
                    })()
                    local_job_schedule.operations[op_idx] = operation
                    local_schedule.n_ops_in_tw += 1
                
                local_schedule.jobs[job_id] = local_job_schedule
        
        return local_schedule

    def _estimate_rough_makespan(self):
        """
        Estimate makespan for rough partitioning method.
        Uses a simple heuristic based on total operations and available resources.
        """
        if not hasattr(self, 'scheduling_instance') or self.scheduling_instance is None:
        ...
        if isinstance(jobs, list):
            jobs_dict = {i: job for i, job in enumerate(jobs)}
            jobs = jobs_dict
        total_operations = sum(job.operations_matrix.shape[0] for job in jobs.values())
        operations_in_segment = total_operations // self.max_window
        if self.current_window == self.max_window - 1:
            operations_in_segment = total_operations - (self.max_window - 1) * (total_operations // self.max_window)
        
        # Rough estimate: operations / (machines + transbots) * average_processing_time
        total_resources = self.num_machines + self.num_transbots
        avg_processing_time = (MAX_PRCS_TIME + MAX_TSPT_TIME) / 2
        estimated_time = (operations_in_segment / total_resources) * avg_processing_time
        
        return max(estimated_time, 100.0)  # Minimum 100 time units
    ...
    def _check_done(self):
        ...
        if isinstance(jobs, list):
            jobs_dict = {i: job for i, job in enumerate(jobs)}
            jobs = jobs_dict
        if all(job.is_completed_for_current_time_window for job in jobs.values()):
    ...
    def _save_instance_snapshot(self, final=False, policy_name="MADRL", episode_id=None, instance_snapshot_dir=None):
        ...
        if instance_snapshot_dir is None:
            ...
            instance_snapshot_dir = current_dir + \
                                    ...
                                    + f"/{policy_name}/snapshot_J{self.num_jobs}I{self.current_instance_id}" \
...
def run_one_local_instance(
    ...
    print(f"Starting instance J{n_jobs}I{scheduling_instance_id}W{current_window}:\n")
    ...
    scheduling_env = NoPartialGlobalPlanMultiAgentEnv(config)

    makespans = []

    for episode_id in range(num_episodes):
        makespans = run_one_episode(
            episode_id=episode_id + 1,
            scheduling_env=scheduling_env,
            makespans=makespans,
            n_jobs=n_jobs, scheduling_instance_id=scheduling_instance_id, 
            max_window=max_window, current_window=current_window,
            do_render=do_render,
...
def run_one_episode(
    ...
    # Generate reset options for NoPartialGlobalPlan environment
    env_reset_options = generate_no_partial_reset_options(
        n_jobs=n_jobs, scheduling_instance_id=scheduling_instance_id, 
        max_window=max_window, current_window=current_window,
        for_training=False,
    )
...
if __name__ == "__main__":
    import time
    from local_realtime_scheduling.BaselineMethods.NoPartialGlobalPlan.generate_no_partial_options import generate_no_partial_reset_options, select_local_instance_randomly, MaxWindowDict
    ...
    current_window = 2
    max_window = MaxWindowDict[f"M{dfjspt_params.n_machines}T{dfjspt_params.n_transbots}"][instance_n_jobs]

    if current_window >= max_window:
        raise ValueError(f"Current window {current_window} is greater than or equal to max window {max_window}!")

    num_instances = 1
    for _ in range(num_instances):
        start_time = time.time()
        run_one_local_instance(
            n_jobs=instance_n_jobs, scheduling_instance_id=scheduling_instance_id, 
            max_window=max_window, current_window=current_window,
            do_plot_gantt=True,
            # do_plot_gantt=False,
            num_episodes=1
